...
## SEP Definition
In the type `dnp3object-type`, zero or one optional data values **MUST** be provided. Which optional property is included is dependent upon the values of the properties, group and variant. The [IEEE-1815](https://standards.ieee.org/findstds/standard/1815-2012.html) standard specifies which optional property.
...
| **objects** (optional)        | `list` of type `dnp3object-header` | This defines the included objects in the Application Layer.                                                                                                                                                  |
...
| prefix (required)     | `integer`   | The prefix code from the Object Qualifier octet.                                    |
| range_spec (required) | `integer`   | The range specifier code from the Object Qualifier octet.                           |
| variant (required)    | `integer`   | The object variation from the Object Type Field.                                    |
| start (optional)      | `integer`   | The start index of the range field, if one is provided.                             |
| stop (optional)       | `integer`   | The stop index of the range field, if one is provided.                              |
| count (optional)      | `integer`   | The count of objects from the range field, if one is provided.                      |
| data (required)       | `list` of `dnp3object-type`   | The list of objects, either requested or provided.                |

### `dnp3object-type`

None or one of the data values **MUST** be present.

| Property Name         | Type        | Description                                                                         |
| -------------         | ----        | -----------                                                                         |
| index (optional)      | `integer`   | The index prefix if the prefix code species one.                                    |
| count (optional)      | `integer`   | The count prefix if the prefix code species one.                                    |
...
Note: SET of n and VARIANT are not yet specified.  If the object has multiple fields, but not a SET of n, this does not yet specify how to code them.
